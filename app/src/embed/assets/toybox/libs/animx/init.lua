...
local function newImage(path)
    return game:getAsset(path)
end

local function old_newImage(path)
...
--[[
	...
	@Params:
		...
		params -  A table of parameters (something I stole from sodapop)
			...
			mode          -  Animation mode; loop, rewind, bounce,rreverse, or once(default)
--]]
function animx.newAnimation(params)
	...
	if type(params)=='string' then
		...
		_img = img
		useImages = params.useImages ~= false and (params.useImages or game.useImages) and img:sub(-4,-4)~="."
		assert(img,"animX Error: Expected Image or URI in `newAnimation`!")
		img = (not useImages) and type(img)=='string' and newImage(img) or img
		
	    nq=params.nq or params.images or params.noOfTiles or params.numberOfQuads or params.noOfQuads or params.noOfFrames

		
		if not useImages then

		sw=params.sw or params.imgWidth or params.imageW or params.textureWidth or img:getWidth()
		sh=params.sh or params.imgHeight or params.imageH or params.textureHeight or img:getHeight()
		end
	...
	local aObj = setmetatable(animation_obj,{__index=Animation})
	aObj._added = treu
	table.insert(animx.animObjs, aObj)--setmetatable(animation_obj,{__index=Animation}))
	
	if useImages then
	    animation_obj:setUseImages(_img,nq)
	end
	
	animation_obj:onAnimStart(onAnimStart):init(startingFrame,delay,true)
	animation_obj
	...
	return aObj--animx.animObjs[#animx.animObjs]
end
...
function Actor:addAnimation(name,anim)
	...
	self.animations[name].actor = self
...
function animx.update(dt)
	...
	for i=1,#animx.animObjs do
	    local obj = animx.animObjs[i]
	    if nil and obj.parent and 
	        ((obj.parent.map and not obj.parent.map.active) or
	         (not obj.parent.map and not obj.parent.active)) then
	        r[#r+1] = obj
	    end
	...
	--[[for x = 1, #r do
	    lume.remove(animx.animObjs, r[x])
	end]]
end
