...
function Animation:init(startingFrame,delay,firstTime)
	self:setDelay(delay)
...
function Animation:setUseImages(folder,nq)
    self.useImages = true
    
    local items = love.filesystem.getDirectoryItems(string.format(game.sourcie,folder))
    for x = 1, (nq or #items) do
        local name = items[x]
        local frame = tonumber(name:sub(1,-5))+1
        self.frames[frame] = --game:getAsset
        (string.format("%s/%s",folder,name))
    end
...
function Animation:destroy()
    if self._added then
        self.destroyed = true
        assert(lume.remove(animx.animObjs, self), "Destroying animation failed")
        return true
    end
...
function Animation:getCurrentQuad()        local f = self.frames[self:getCurrentFrame()]
    if type(f) == "string" then
        local asset = game:getAsset(f)
        self.frames[self:getCurrentFrame()] = asset
        return asset
    end
...
function Animation:setFrame(frame)
    self.curFrame = math.max(1,math.min(frame,self:getSize()))
    return self
end
...
function Animation:update(dt)
    local dt = (self.parent and self.parent.dt) or dt
...
-- like normal render put temporarily set a different texture source
function Animation:renderTexture(texture,x,y,r,sx,sy,ox,oy,...)
	sx,sy=sx or 1, sy or 1
	sx=self.p_flipX and -sx or sx
	sy=self.p_flipY and -sy or sy
	if not self.useImages then
    	love.graphics.draw(
	    	texture,self:getCurrentQuad(),
	    	x,y,r,sx,sy,ox,oy,...
    	)
    else
        love.graphics.draw(
...
Animation.drawTexture=Animation.renderTexture
