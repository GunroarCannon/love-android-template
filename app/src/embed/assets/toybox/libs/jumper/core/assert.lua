...
if (...) then
	...
	local function arrayContentsMatch(t,...)
		...
		if n_count < 1 then er() return false end
		...
		for i=init_count,n_count,1 do
			if not t[i] then error(i..","..n_count) return false end
			if types then
				if not types:match(type(t[i])) then er() return false end
			end
		end
		return true
	end	
  ...
  local function isMap(m)
		...
		for k,v in pairs(m) do
			if not arrayContentsMatch(m[k], 'string', 'int','table',"userdata") then return false end
