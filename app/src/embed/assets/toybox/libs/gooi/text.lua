...
function gooi.newText(params)
    ...
    function f:drawSpecifics(fg)
        ...
        love.graphics.setColor(self.insideColor or colors.black)--0, 0, 0)
        ...
        love.graphics.setColor(self.fontColor or fg)
    ...
    function f:typeText(text)
        if love.keyboard.isDown("lctrl") or love.keyboard.isDown("rctrl") then
            if text == "v" then
                self:setText(string.format("%s%s", self:getText() or "", love.system.getClipboardText() or ""))
                return
            elseif text == "c" then
        ...
        if self.onlyNumbers and not tonumber(text) then
            return
        end
        ...
        self:fixViewport()
        self.r(self:getText(),self)
    end

    function f:moveLeft()
        if self.indexCursor > 0 then--and gooi.desktop then
            self.indexCursor = self.indexCursor - 1
        end
        self:fixViewport()
    end
    ...
    function f:moveRight()
        if self.indexCursor < #self.letters then--and gooi.desktop then
            self.indexCursor = self.indexCursor + 1
        end
        self:fixViewport()
    end
    ...
    function f:deleteBack()
        if #self.letters > 0 then--and gooi.desktop then
    ...
    function f:onPress(func)
        self.r = func
        return self
    end
    
    function f:onValue(func)
        self.r = func
        return self
    end
    
    function f:onText(func)
        self.r = func
        return self
    end
