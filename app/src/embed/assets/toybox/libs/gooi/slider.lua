...
function gooi.newSlider(params)
  ...
  function s:updateGUI()
    ...
    if self._onSlide and self.displacement ~= 0 then self:_onSlide(self.value) end
  end
  function s:onSlide(f)
    self._onSlide = f
    return self
  end
  ...
  function s:getValue(round)
    return gooi.round(self.value, round or 2)
  end
  function s:rebuild()
    --self:generateBorder()
