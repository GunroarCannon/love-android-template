...
function layout.new(specs)
	...
	if l.specs:sub(0, 4) == "grid" then
		...
		function l:nextCell(c)
			...
			local count = 0
			while not generateNext() and count<100 do count = count+1 end-- Ignore the not usable cells.
			if count >= 100 then error("Problem in layout cells.") end
			...
			for theRow = 1, self.gridRows do
				...
				for theCol = 1, self.gridCols do
					self.gridCells[theRow][theCol] =
					{
						...
						y = math.floor(panel.y + ((theRow - 1) * fh / self.gridRows)) + (self.paddingY or pad/2)*2,
						w = (fw / self.gridCols) - pad,
						h = (fh / self.gridRows) - (self.paddingY or pad/2)*2,
						row = theRow,
						col = theCol,
						rowspan = 1,
						colspan = 1,
