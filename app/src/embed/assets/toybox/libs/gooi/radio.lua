...
function gooi.newRadio(params)
  ...
  r:rebuild()
  r._onSelected = function() end
  function r:onSelected(func)
      self._onSelected = func
      return self
  end
  function r:drawSpecifics(fg)
    ...
    if self.selected then
      love.graphics.setColor(self.fillColor or fg)
    end
    gooi.drawInnerShape(self, "fill", mC, side)

    love.graphics.setColor(fg)
    if self.selected then
      love.graphics.setColor(self.outlineColor or colors.black)
    end
