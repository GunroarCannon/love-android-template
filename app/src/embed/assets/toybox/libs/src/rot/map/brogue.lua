...
function Brogue:init(width, height, options)
    ...
    self._options={
                    ...
                    maxRooms=99,
                    minRooms=1,
                    roomIsLockedChance = .10,
                    maxLockedRooms = 1,
                    minLockedRooms = 0,--Not used for now
                  }
    ...
    self._maxrooms=self._options.maxRooms
    self._minrooms=self._options.minRooms
...
function Brogue:_buildCave()
    ...
    self.isCave = true
    cl:randomize(.55)
    for _=1,(self._options.cavePasses or 5) do cl:create() end
    local map=cl._map
    local id=2
    local largest=2
...
function Brogue:_generateRooms(stop, rooms)
    local rooms=rooms or 0+1 -- first room
    local rr=nil
    for i=1,self._rng:random(self._minrooms,self._maxrooms) do
        if rooms>=self._maxrooms then return end--break end
        local r =  self:_buildRoom(i>375)
        rr=r
        if r then
            rooms=rooms+1
            log(string.format("[BROGUE MAP BUILDER] Generated ROOM NUMBER %s", rooms))
        end
    end

    if rooms<self._minrooms and rr then self:_generateRooms(nil, rooms)
    elseif rooms<self._minrooms and not stop then self:_generateRooms(true, rooms) end
end
        ...
        if self._rng:random()<self._options.corridorChance and not forceNoCorridor then
            ...
            if corridor:isValid(self._isWallCallback, self._canBeDugCallback) then
                ...
                if room:isValid(self._isWallCallback, self._canBeDugCallback) then
                    ...
                    --Edited
                    local locked = self._options.roomIsLockedChance
                    if self._rng:random() < locked then
                        if self._lockedRooms <= self._options.maxLockedRooms then
                            room.isLocked = true
                            self._lockedRooms = self._lockedRooms + 1
                            table.insert(self.lockedRooms,room)
                        end
                    end
                    
                    --Edited
                    if not room.isLocked then
                        corridor:create(self._digCallback)
                        table.insert(self._corridors, corridor)
                    end
                    if room.isLocked and #room.corridors <= 0 then
                        corridor:create(self._digCallback)
                        table.insert(self._corridors, corridor)
                    end
...
function Brogue:_digCallback(x, y, value, corridor)
...
function Brogue:_canBeDugCallback(x, y)
    ...
    for i=1,#drs do
        if not self._map[x+drs[i][1]] then return false end 
...
function Brogue:getLockedRooms()
    return self.lockedRooms
end
