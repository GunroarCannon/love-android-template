...
function Actor:switch(newState)
	...
	if self:isActive() then self:stopAnimation(true) end
...
function Actor:destroy()
    for k, i in ipairs(self.animations) do
        i:destroy()
    end
...
function Actor:stopAnimation(fake)
	if self:getCurrentAnimation() then
		self:getCurrentAnimation():stop(fake)
	end
	return self
end
...
--!!Renfer method edited by hmmmgames!!--
function Actor:render(...)
	local anim = self:getCurrentAnimation()
	 anim.parent = self.parent
	anim:render(...)
	--anim.parent = nil
end
